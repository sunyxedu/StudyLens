...
1. Ingestion adapters collect Imperial course artifacts. The auto-index pipeline discovers Scientia courses, downloads supported materials/exercises/tutorials, extracts text, and indexes chunks. It also searches Panopto, prefers caption/subtitle indexing with timestamps, and falls back to video transcription when captions are unavailable and a downloadable video plus OpenAI key are available. Site-specific login and browser automation stays in `studylens.ingestion`.
